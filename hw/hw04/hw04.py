...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    ###触底反弹return后，对应的t也会返回到上一层。
    ###比如此时的t为最后一层节点，那么return以后，返回到的definition scope里的t就是倒数第二层某一个节点的t了

    highest = 0
    for b in branches(t):
        highest = max(max_path_sum(b), highest) ###当t到底层触发return后，又会回到这里，并且把最后一层的节点遍历完
    return label(t) + highest 
    #只有当变量对应的definition scope执行完才会执行return
    #也就是说只有t对应下面的节点遍历完才会执行return
