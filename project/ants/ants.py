...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curplace = self.place
        distance = 0
        while not curplace.is_hive and distance < self.lower_bound:
            curplace = curplace.entrance
            distance += 1
        while not curplace.is_hive and distance <= self.upper_bound:
            if len(curplace.bees) > 0:
                break
            curplace = curplace.entrance
            distance += 1

        if curplace.is_hive or distance > self.upper_bound:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        def reflected_image(amount):
            exist_bees = []
            for bee in self.place.bees:
                if bee.health > amount:
                    exist_bees.append(bee)
            
            for bee in self.place.bees.copy():
            ###当你在遍历列表或字典同时进行内容的修改，最好要用copy()或者新创建一个容器
            ###否则很容易出错
                Insect.reduce_health(bee, amount)

            self.place.bees = exist_bees

        reflected_image(amount)
        if self.health <= amount:
            reflected_image(self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees and self.turns_to_chew <= 0:
            bee_killed = random_bee(self.place.bees)

            Insect.reduce_health(bee_killed, bee_killed.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained == None and other.is_container is False:
            return True
        return False
        # END Problem 8a
        ...
        if True:
            ...
            while curr_place is not None:
                ...
                and not curr_place.ant.ant_contained.is_doubled:
                    ...
                    curr_place.ant.ant_contained.is_dounbled = True
