...
def pick(paragraphs, select, k):
    ...
    for i in range(0,len(paragraphs)):
        if select(paragraphs[i]): index_in.append(i)
    for i in index_in:
        new_para.append(paragraphs[i])

    if k >= len(new_para): return ''
    else: return new_para[k]
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    elif len(typed_words) == 0: return 0.0
    res = correct / len(typed_words) * 100
    return res
    # END PROBLEM 3
